...
rule counting_reads_in_trimmed_peaks:
    """Counting reads per barcode"""
    input:
        bams = join(OUT_DIR, '{sample}', "{reference}", 'mapping', "sample.barcoded.minmapq{minmapq}.dedup.mincount{mincounts}.bam"),
        bai = join(OUT_DIR, '{sample}', "{reference}", 'mapping', "sample.barcoded.minmapq{minmapq}.dedup.mincount{mincounts}.bam.bai"),
...
INPUT_ALL.append(expand(rules.counting_reads_in_trimmed_peaks.output,
                        reference=config['reference'],
                        sample=samples.Name.tolist(),
...
rule counting_reads_in_original_peaks:
    """Counting reads per barcode"""
    input:
        bams = join(OUT_DIR, '{sample}', "{reference}", 'mapping', "sample.barcoded.minmapq{minmapq}.dedup.mincount{mincounts}.bam"),
        bai = join(OUT_DIR, '{sample}', "{reference}", 'mapping', "sample.barcoded.minmapq{minmapq}.dedup.mincount{mincounts}.bam.bai"),
        regions = join(OUT_DIR, '{sample}', "{reference}", "peaks", "sample.minmapq{minmapq}.mincount{mincounts}_peaks.narrowPeak")
    output: join(OUT_DIR, '{sample}', '{reference}', 'countmatrix', 'original_genrichpeakcounts.minmapq{minmapq}.mincount{mincounts}.tab'),
            join(OUT_DIR, '{sample}', '{reference}', 'countmatrix', 'original_genrichpeakcounts.minmapq{minmapq}.mincount{mincounts}.tab.counts')
    wildcard_constraints:
       minmapq='\d+', mincounts='\d+'
    params:
        count_both_ends=config['count_both_ends']
    resources:
        mem_mb=10000
    log: join(LOG_DIR, 'genrichpeak_countmatrix_{sample}_{reference}.minmap{minmapq}.mincount{mincounts}.log')
    run: 
        try:
            sparse_count_reads_in_regions(input.bams, input.regions, output[0], flank=0, log=log[0], count_both_ends=params.count_both_ends)
        except:
...
INPUT_ALL.append(expand(rules.counting_reads_in_original_peaks.output,
                        reference=config['reference'],
                        sample=samples.Name.tolist(),
                        minmapq=config['min_mapq'],
                        mincounts=config['min_counts_per_barcode']))
